...
public class CuponSWAP {
	...
	public void calcularTnaIndex() throws Exception {
		if (this.getFechaIndiceFin().compareTo(this.getFechaProceso()) <= 0) {//todas las tasas son anteriores (Badlar)
			this.setTnaIndex(DAO.obtenerPromedioTasasDeBadlar(this.getFechaIndiceInicio(), this
					.getFechaIndiceFin()));
		} else {
			if (this.getFechaIndiceInicio().compareTo(this.getFechaProceso()) > 0) {//todas las tasas son posteriores (FWD)
				this.setTnaIndex(DAO.obtenerPromedioTasasFWD(this.getFechaIndiceInicio(), this
						.getFechaIndiceFin()));
			} else {
				//algunas tasas son anteriores (Badlar) y otras son posteriores (FWD)
	...
	public void calcularVFutCli() throws ParseException {
		this.setVFutCli(this.getCantidadVN()
				...
				* (DateUtils.diferenciaEntreFechas(this.getFechaVencimiento(), this
						.getFechaInicio())) / this.getBase() * 100);
	}
